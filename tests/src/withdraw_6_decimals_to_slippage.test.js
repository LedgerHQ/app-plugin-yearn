...
// Nanos S test
test('[Nano S] Withdraw Tokens with Recipient and Slippage 6 decimals', zemu("nanos", async (sim, eth) => {
  ...
  await sim.navigateAndCompareSnapshots('.', 'nanos_withdraw_recipient_splippage_6', [BASE_SCREENS_S + 3 + 1, 0]);
  await tx;
}));

test('[Nano X] Withdraw Tokens with Recipient and Slippage 6 decimals', zemu("nanox", async (sim, eth) => {
  const contract = new ethers.Contract(contractAddr, ['function withdraw(uint256, address, uint256)']);
  const {data} = await contract.populateTransaction.withdraw(AMOUNT_TO_DEPOSIT, recipient, 1000);
  ...
  await waitForAppScreen(sim);
  await sim.navigateAndCompareSnapshots('.', 'nanox_withdraw_recipient_splippage_6', [BASE_SCREENS_X + 1 + 1, 0]);
  await tx;
}));

test('[Nano SP] Withdraw Tokens with Recipient and Slippage 6 decimals', zemu("nanosp", async (sim, eth) => {
  const contract = new ethers.Contract(contractAddr, ['function withdraw(uint256, address, uint256)']);
  const {data} = await contract.populateTransaction.withdraw(AMOUNT_TO_DEPOSIT, recipient, 1000);
