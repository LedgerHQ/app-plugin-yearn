...
nano_models.forEach(function(model) {
  test('[Nano ' + model.letter + '] Withdraw Tokens with Recipient and Slippage 6 decimals', zemu(model, async (sim, eth) => {
    ...
    await sim.navigateAndCompareSnapshots('.', model.name + '_withdraw_recipient_splippage_6', [right_clicks, 0]);
    await tx;
  }));
});
