...
test('[Nano S] Deposit Tokens with Recipient', zemu("nanos", async (sim, eth) => {
  ...
  await sim.navigateAndCompareSnapshots('.', 'nanos_deposit_recipient_6', [BASE_SCREENS_S + 3, 0]);
  await tx;
}));
...
test('[Nano X] Deposit Tokens with Recipient', zemu("nanox", async (sim, eth) => {
  ...
  await waitForAppScreen(sim);
  await sim.navigateAndCompareSnapshots('.', 'nanox_deposit_recipient_6', [BASE_SCREENS_X + 1, 0]);
  await tx;
}));
