...
// Nanos S test
test('[Nano S] Deposit Tokens with Recipient 18 decimals', zemu("nanos", async (sim, eth) => {
  ...
  await sim.navigateAndCompareSnapshots('.', 'nanos_deposit_recipient_18', [BASE_SCREENS_S + 3, 0]);
  await tx;
}));

test('[Nano X] Deposit Tokens with Recipient 18 decimals', zemu("nanox", async (sim, eth) => {
  const contract = new ethers.Contract(contractAddr, ['function deposit(uint256, address)']);
  const {data} = await contract.populateTransaction.deposit(AMOUNT_TO_DEPOSIT, recipient);
  ...
  await waitForAppScreen(sim);
  await sim.navigateAndCompareSnapshots('.', 'nanox_deposit_recipient_18', [BASE_SCREENS_X + 1, 0]);
  await tx;
}));
